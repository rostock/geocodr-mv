    ...
    '\'', '’', PatternReplace(
      '´', '’', PatternReplace(
        '`', '’', PatternReplace(
          '‘', '’', PatternReplace(
            '\'', '’', PatternReplace(
              r'(?i)str(a((ß|ss?)e?)?)?\b', 'str.', PatternReplace(
                r'\Bstr.', ' str.', field)))))))
...
class Strassen(Collection):
  ...
  qfields = (
    replace_strasse(NGramField('strasse_name_ngram') ^ 1.6),
...
class Adressen(Collection):
  ...
  qfields = (
    ...
    replace_strasse(NGramField('strasse_name_ngram') ^ 0.8),
    ...
    Only(r'^\d{1,3}[a-zA-Z]{,2}$', PrefixField('hausnummer', 1)),
    # search for zip codes only in postleitzahl
    Only(r'^\d{3,5}$', PrefixField('postleitzahl')),
  )
...
class StrassenHro(Collection):
  ...
  def to_title(self, prop):
    parts = [prop['gemeindeteil_name'], prop['strasse_name']]
    return ', '.join(parts)
...
class AdressenHro(Collection):
  ...
  def to_title(self, prop):
    parts = [prop['gemeindeteil_name'],
             prop['strasse_name'] + ' ' + prop['hausnummer'] + (prop['hausnummer_zusatz'] or '')]
    if 'historisch_seit' in prop and prop['historisch_seit']:
      parts[-1] += ' – historisch seit ' + datetime.strptime(prop['historisch_seit'],
                                                             '%Y-%m-%d').strftime('%d.%m.%Y')
    elif 'gueltigkeit_bis' in prop and prop['gueltigkeit_bis']:
      parts[-1] += ' – historisch seit ' + datetime.strptime(prop['gueltigkeit_bis'],
                                                             '%Y-%m-%d').strftime('%d.%m.%Y')
    return ', '.join(parts)
...
class FlurenHro(Collection):
  ...
  def to_title(self, prop):
    parts = [prop['gemarkung_name'] + ' (' + prop['gemarkung_schluessel'][2:] + ')',
             'Flur ' + str(int(prop['flur'], 10))]
    return ', '.join(parts)
...
class FlurstueckeHro(Collection):
  ...
  def to_title(self, prop):
    parts = [prop['gemarkung_name'] + ' (' + prop['gemarkung_schluessel'][2:] + ')',
             'Flur ' + str(int(prop['flur'], 10)), str(int(prop['zaehler'], 10))]
    if prop['nenner'] != '0000':
      parts[-1] += '/' + str(int(prop['nenner'], 10))
    if 'historisch_seit' in prop and prop['historisch_seit']:
      parts[-1] += ' – historisch seit ' + datetime.strptime(prop['historisch_seit'],
...
class Postleitzahlengebiete(Collection):
  ...
  fields = ('postleitzahl',)
  qfields = (
    Only(r'^\d{4,5}$', PrefixField('postleitzahl')),
  )
